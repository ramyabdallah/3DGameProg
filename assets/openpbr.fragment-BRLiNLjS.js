...
`;t.IncludesShadersStore[n]||(t.IncludesShadersStore[n]=r);var i=`openpbrFragmentSamplersDeclaration`,a=`#include<samplerFragmentDeclaration>(_DEFINENAME_,BASE_COLOR,_VARYINGNAME_,BaseColor,_SAMPLERNAME_,baseColor)
...
#endif
`;t.IncludesShadersStore[i]||(t.IncludesShadersStore[i]=a);var o=`openpbrNormalMapFragmentMainFunctions`,s=`#if defined(GEOMETRY_NORMAL) || defined(GEOMETRY_COAT_NORMAL) || defined(ANISOTROPIC) || defined(FUZZ) || defined(DETAIL)
...
#endif
`;t.IncludesShadersStore[o]||(t.IncludesShadersStore[o]=s);var c=`openpbrNormalMapFragmentFunctions`,l=`#if defined(GEOMETRY_NORMAL)
...
#endif
`;t.IncludesShadersStore[c]||(t.IncludesShadersStore[c]=l);var u=`openpbrConductorReflectance`,d=`#define pbr_inline
...
#endif
outParams.F0=1.0;outParams.F90=1.0;return outParams;}`;t.IncludesShadersStore[u]||(t.IncludesShadersStore[u]=d);var f=`openpbrAmbientOcclusionFunctions`,p=`float compute_specular_occlusion(const float n_dot_v,const float metallic,const float ambient_occlusion,const float roughness)
...
`;t.IncludesShadersStore[f]||(t.IncludesShadersStore[f]=p);var m=`openpbrVolumeFunctions`,h=`struct OpenPBRHomogeneousVolume {vec3 extinction_coeff; 
vec3 ss_albedo; 
...
weight_sum.b<1e-5f ? unconvolved_irradiance.b : irradiance_sum.b/weight_sum.b);}`;t.IncludesShadersStore[m]||(t.IncludesShadersStore[m]=h);var g=`openpbrNormalMapFragment`,_=`vec2 uvOffset=vec2(0.0,0.0);
...
#endif
`;t.IncludesShadersStore[g]||(t.IncludesShadersStore[g]=_);var v=`openpbrBlockNormalFinal`,y=`#if defined(FORCENORMALFORWARD) && defined(NORMAL)
...
#endif
#endif
`;t.IncludesShadersStore[v]||(t.IncludesShadersStore[v]=y);var b=`openpbrBaseLayerData`,x=`vec3 base_color=vec3(0.8);float base_metalness=0.0;float base_diffuse_roughness=0.0;float specular_weight=1.0;float specular_roughness=0.3;vec3 specular_color=vec3(1.0);float specular_roughness_anisotropy=0.0;float specular_ior=1.5;float alpha=1.0;vec2 geometry_tangent=vec2(1.0,0.0);float geometry_thickness=0.0;
#ifdef BASE_WEIGHT
...
#endif
`;t.IncludesShadersStore[b]||(t.IncludesShadersStore[b]=x);var S=`openpbrCoatLayerData`,C=`float coat_weight=0.0;vec3 coat_color=vec3(1.0);float coat_roughness=0.0;float coat_roughness_anisotropy=0.0;float coat_ior=1.6;float coat_darkening=1.0;vec2 geometry_coat_tangent=vec2(1.0,0.0);
#ifdef COAT_WEIGHT
...
#endif
`;t.IncludesShadersStore[S]||(t.IncludesShadersStore[S]=C);var w=`openpbrThinFilmLayerData`,T=`#ifdef THIN_FILM
...
#endif
`;t.IncludesShadersStore[w]||(t.IncludesShadersStore[w]=T);var E=`openpbrFuzzLayerData`,D=`float fuzz_weight=0.0;vec3 fuzz_color=vec3(1.0);float fuzz_roughness=0.0;
#ifdef FUZZ
#ifdef FUZZ_WEIGHT
...
#endif
#endif
`;t.IncludesShadersStore[E]||(t.IncludesShadersStore[E]=D);var O=`openpbrAmbientOcclusionData`,k=`vec3 ambient_occlusion=vec3(1.0);float specular_ambient_occlusion=1.0;float coat_specular_ambient_occlusion=1.0;
#ifdef AMBIENT_OCCLUSION
...
#endif
`;t.IncludesShadersStore[O]||(t.IncludesShadersStore[O]=k);var A=`openpbrBackgroundTransmission`,j=`vec4 slab_translucent_background=vec4(0.,0.,0.,1.);
#ifdef REFRACTED_BACKGROUND
...
#endif
`;t.IncludesShadersStore[A]||(t.IncludesShadersStore[A]=j);var M=`openpbrEnvironmentLighting`,N=`#if defined(REFLECTION) || defined(REFRACTED_BACKGROUND)
...
#endif
`;t.IncludesShadersStore[M]||(t.IncludesShadersStore[M]=N);var P=`openpbrDirectLightingInit`,F=`#ifdef LIGHT{X}
...
#endif
#endif
`;t.IncludesShadersStore[P]||(t.IncludesShadersStore[P]=F);var I=`openpbrDirectLighting`,L=`#ifdef LIGHT{X}
...
#endif
`;t.IncludesShadersStore[I]||(t.IncludesShadersStore[I]=L);var R=`openpbrBlockPrePass`,z=`#if SCENE_MRT_COUNT>0
...
#endif
#endif
`;t.IncludesShadersStore[R]||(t.IncludesShadersStore[R]=z);var B=e({openpbrPixelShader:()=>U}),V=`openpbrPixelShader`,H=`#define OPENPBR_FRAGMENT_SHADER
...
`;t.ShadersStore[V]||(t.ShadersStore[V]=H);var U={name:V,shader:H};export{B as t};